...
class Logicoder_DB_DataDict
{
     ...
     */
    public $sCreateTable    = 'CREATE TABLE `%s`';
    public $sCreateTableIf  = 'CREATE TABLE IF NOT EXISTS `%s`';
    public $sAlterTable     = 'ALTER TABLE `%s`';
    public $sRenameTable    = 'RENAME TABLE `%s` TO `%s`';
    public $sDropTable      = 'DROP TABLE `%s`';
    public $sDropTableIf    = 'DROP TABLE IF EXISTS `%s`';
    ...
    public $sCreatePrimary  = 'ADD PRIMARY KEY';
    ...
    {
        $sql = ($bDrop) ? sprintf($this->sDropTableIf, $sTableName) . ";\n" : '';
        if ($bIfNotExists)
        {
            $sql .= sprintf($this->sCreateTableIf, $sTableName) . ' (';
        }
        else
        {
     ...
     * Drop a table.
     */
    public function drop_table ( $sTableName, $bIfExists = false )
    {
        if ($bIfExists)
        {
            return sprintf($this->sDropTableIf, $sTableName);
        }
        else
        {
     ...
     */
    public function create_primary_index ( $sTableName, array $aFields )
    {
        /*
            Return SQL generated by method.
        */
        return $this->create_index(null, $sTableName, $aFields, $this->sCreatePrimary);
    }
